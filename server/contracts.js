...
class ContractManager {
  ...
  // Get contracts by session ID (alias for getSessionContracts)
  async getContractsBySessionId(sessionId) {
    return this.getSessionContracts(sessionId);
  }

  // Create custom contract
  async createCustomContract(sessionId, customData, session) {
    try {
      const contractId = uuidv4();
      const accessToken = this.generateAccessToken();
      
      // Replace template variables in custom content
      const processedContent = customData.content.replace(/\{\{(\w+)\}\}/g, (match, key) => {
        const replacements = {
          client_name: session.clientName || session.client_name,
          ...
          session_date: new Date(session.dateTime || session.date_time).toLocaleDateString(),
      ...
      const query = `
        INSERT INTO contracts (
          id, session_id, user_id, contract_type, contract_title, contract_content,
          status, client_name, client_email, photographer_name, photographer_email,
          access_token, custom_fields, created_at, updated_at
      ...
      const result = await pool.query(query, [
        ...
        processedContent,
        'pending',
        session.clientName || session.client_name,
        session.email,
        'Lance Casselman',
      ...
      console.log(`Custom contract created: ${customData.title} for session ${sessionId}`);
      return result.rows[0];
    } catch (error) {
  ...
  // Update contract with flexible fields
  async updateContractById(contractId, updates) {
    try {
      const setClause = [];
      const values = [];
      let paramCount = 1;
