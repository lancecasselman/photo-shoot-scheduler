...
- **Universal File Management** (Updated Feb 3, 2025): Complete file upload system supporting all file types - photos, RAW files, videos, documents, audio files, Adobe files (PSD, AI, INDD), and more. Features dual storage architecture with original full-size files uploaded to Firebase Storage for client downloads, and optimized versions (images only, 1920x2560 max, 85% quality) stored locally for fast app display. Includes intelligent file type detection, visual file type indicators with emoji icons (🖼️📄🎬🎵📁), type-specific badges, sequential processing, real-time progress tracking, individual file deletion, and lightbox viewing. RAW file detection expanded to include video formats (.mov, .mp4, .r3d, .braw), audio files (.wav, .flac, .aiff), and professional documents for comprehensive photographer workflow support.
